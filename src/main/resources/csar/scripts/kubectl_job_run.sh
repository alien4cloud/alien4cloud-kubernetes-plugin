...
# Get failed jobs
failedJobs=$(kubectl --kubeconfig "${KUBE_ADMIN_CONFIG_PATH}"  get "${TOSCA_JOB_ID}" -o=jsonpath={.status.failed})
if [[ -z "${failedJobs}" ]] ; then 
    failedJobs=0
fi

# Get succeeded jobs
succeededJobs=$(kubectl --kubeconfig "${KUBE_ADMIN_CONFIG_PATH}"  get "${TOSCA_JOB_ID}" -o=jsonpath={.status.succeeded})
if [[ -z "${succeededJobs}" ]] ; then 
    succeededJobs=0
fi

# Get expected completions
expectedCompletions=$(kubectl --kubeconfig "${KUBE_ADMIN_CONFIG_PATH}"  get "${TOSCA_JOB_ID}" -o=jsonpath={.spec.completions})
...
pods=$(kubectl --kubeconfig "${KUBE_ADMIN_CONFIG_PATH}"  get pods --selector=job-name=${TOSCA_JOB_ID##*/} --output=jsonpath={.items..metadata.name})
for pod in ${pods} ; do
